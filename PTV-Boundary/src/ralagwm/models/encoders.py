...
class ImageEncoder(nn.Module):
    def __init__(self, image_size: int, image_channels: int, hidden_dim: int, conv_channels: Sequence[int] = (32, 64, 128)) -> None:
        super().__init__()
        ...
        self.proj = nn.Sequential(nn.Flatten(), nn.LazyLinear(int(hidden_dim)), nn.ReLU())
        self.image_size = int(image_size)
        self.image_channels = int(image_channels)
        self.hidden_dim = int(hidden_dim)
    def forward(self, obs: torch.Tensor) -> torch.Tensor:
        obs = _ensure_bchw(obs, expected_channels=self.image_channels)
        feat = self.conv(obs)
        return self.proj(feat)


def build_encoder(obs_type: str, obs_dim: int, hidden_dim: int, image_size: int = 84, image_channels: int = 3) -> nn.Module:
    if obs_type == "image":
        return ImageEncoder(image_size=image_size, image_channels=image_channels, hidden_dim=hidden_dim)
    return ProprioEncoder(obs_dim=obs_dim, hidden_dim=hidden_dim)
